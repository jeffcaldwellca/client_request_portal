...
$username_err = $password_err = $level_err = $fullname_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($username_err) && empty($password_err)){
        ...
        $sql = "SELECT id, username, password, level, fullname FROM users WHERE username = ?";
        ...
        if($stmt = $mysqli->prepare($sql)){
            ...
            if($stmt->execute()){
                ...
                if($stmt->num_rows == 1){
                    ...
                    $stmt->bind_result($id, $username, $hashed_password, $level, $fullname);
                    if($stmt->fetch()){
